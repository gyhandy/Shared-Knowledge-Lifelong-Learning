...
class ProtoTaskMapper():
    ...
    def fit_task(self, n_c, taskID, task_features, **llmkwargs):
        '''
        ...
        if self.n_C_min < self.n_C_max:
            ...
            #n_c = self.n_C_min
            # log(f"gmmc_{n_c}", f"this task has: {n_c}")
            gmm_ind=0
        ...
        # log(f"gmmc_{n_c}", 'Generate %d New Centroids'%gmm_model_new[0].shape[0])
        ...
        # log(f"gmmc_{n_c}", f"All Centroids so far: {self.ProtoMap.w.shape[0]} {self.ProtoMap.LLClustering.weights_} {sum(self.ProtoMap.LLClustering.weights_)}")    
